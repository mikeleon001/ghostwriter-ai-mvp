...
public class JSONDecorator extends SummaryFormatterDecorator {
    ...
    public String format(Summary summary) {
        ...
        if (stats.containsKey(StatisticsKeys.MOST_ACTIVE)) {
            sb.append(",\n");
            sb.append("    \"mostActive\": ").append(toJsonString(stats.get(StatisticsKeys.MOST_ACTIVE).toString()));
        }
        
        if (stats.containsKey(StatisticsKeys.AVG_MESSAGE_LENGTH)) {
            sb.append(",\n");
            sb.append("    \"avgMessageLength\": ").append(stats.get(StatisticsKeys.AVG_MESSAGE_LENGTH));
        }
